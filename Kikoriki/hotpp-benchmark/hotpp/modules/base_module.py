...
# next-item metrics
from ..metrics.next_item import NextItemMetric
from ..metrics.next_item_ranking import NextItemRankingMetric
...
class BaseModule(pl.LightningModule, ABC):
    ...
    def __init__(
        ...
        self._head = (
            head_partial(seq_encoder.hidden_size, loss.input_size)
    ...
    def predict_next(
        ...
        """
        results = self._loss.predict_next(
    ...
    def embed(self, x):
        ...
        hiddens, states = self._seq_encoder(
            x, return_states="full" if self._aggregator.need_states else False
        )  # (B, L, D).
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self._val_metric is not None:
            # IMPORTANT: pass targets y too
            self._update_metric(self._val_metric, x, y, outputs, states)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self._test_metric is not None:
            # IMPORTANT: pass targets y too
            self._update_metric(self._test_metric, x, y, outputs, states)
    ...
    def configure_optimizers(self):
        ...
        if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
            scheduler = {"scheduler": scheduler, "monitor": "val/loss"}
    ...
    def _update_metric(self, metric, features, targets, outputs, states):
        """
        metric: HorizonMetric (старое) ИЛИ NextItemMetric/NextItemRankingMetric (только next-item)
        features: x
        targets: y (нужен для target_labels/target_timestamps)
        """
        ...
        # ---- NEXT-ITEM METRICS PATH (uses global target fields) ----
        if isinstance(metric, (NextItemMetric, NextItemRankingMetric)):
            if ("target_labels" not in targets.payload) or ("target_timestamps" not in targets.payload):
                raise RuntimeError(
                    "NextItem* metric requires targets.payload['target_labels'] and ['target_timestamps'].\n"
                    "Make sure you pass:\n"
            ...
            # valid mask: label != -1 and timestamp finite
            ...
            # NextItemMetric expects these names in update(...)
            metric.update(
                mask=mask.to(device),
        ...
        # ---- ORIGINAL HORIZONMETRIC PATH ----
        metric.update_next_item(
            lengths,
    ...
    def _get_grad_norm(self):
        named = [pair for pair in self.named_parameters() if pair[1].requires_grad]
        if len(named) == 0:
            return 0.0
