...
from .dataset import HotppDataset, ShuffledDistributedDataset, HotppCSVDataset
...
def make_dataset(path, params):
    if isinstance(path, str) and path.endswith(".csv"):
        return HotppCSVDataset(path, **params)
    return HotppDataset(path, **params)
...
class HotppDataModule(pl.LightningDataModule):
    def __init__(self,
        ...
        if train_path is not None:
            ...
            self.train_data = make_dataset(train_path, train_params)
        else:
        ...
        if val_path is not None:
            ...
            self.val_data = make_dataset(val_path, val_params)
        else:
            self.val_data = None
        if test_path is not None:
            ...
            self.test_data  = make_dataset(test_path,  test_params)
