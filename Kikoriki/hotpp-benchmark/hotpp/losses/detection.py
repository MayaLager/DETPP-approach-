...
class DetectionLoss(NextKLoss):
    ...
    Args:
        ...
        categorical_fields: The list of categorical features.
        ...
        drop_partial_windows: Compute the loss only for full-horizon ground truth windows.
            Possible values: True, False, and "calibration".
        ...
        next_item_adapter: Adapter names for next-item predictions.
        next_item_loss_weight: The weight of the adapter-based next-item loss.
        next_item_trainable_scales: Whether to learn affine calibration for next-item outputs.
        presence_loss_weight: Multiplier for presence BCE loss term (1.0 = default, 0.0 = disable BCE training).
    """
    ...
    def __init__(
        ...
        presence_loss_weight: float = 1.0,  # ✅ NEW
    ):
        super().__init__(
        ...
        # ✅ NEW: enables easy “with/without BCE” switching
        self._presence_loss_weight = float(presence_loss_weight)
        ...
        if next_item_trainable_scales:
            self.next_offsets = torch.nn.ModuleDict(
                {k: torch.nn.Parameter(torch.zeros([])) for k in self.data_fields}
            )
            self.next_scales = torch.nn.ModuleDict(
                {k: torch.nn.Parameter(torch.ones([])) for k in self.data_fields}
            )
        else:
    ...
    def forward(self, inputs, outputs, states):
        ...
        if self.training:
            with torch.no_grad():
                ...
                reshaped_outputs = PaddedBatch(
                    ...
                    torch.full([b * l], self._k, dtype=torch.long, device=outputs.device),
                )  # (BL, K, P).
                ...
                if self._drop_partial_windows in {True, "calibration"}:
                    ...
                    presence_logits = PaddedBatch(
                        presence_logits, (outputs.seq_lens - self._prefetch_k).clip(min=0)
                    )
                else:
        ...
        losses = {k: v.take_along_dim(matching.unsqueeze(3), 3).squeeze(3) for k, v in losses.items()}  # (B, I, K).
        ...
        # ✅ CHANGED: multiply by self._presence_loss_weight (0 disables BCE training term)
        losses["_presence"] = presence_losses[index_mask].mean() * self._presence_loss_weight

        # Optional: log the chosen weight so you see it in CSV
        matching_metrics["presence_loss_weight"] = torch.tensor(
            self._presence_loss_weight, device=outputs.device, dtype=torch.float32
        )
        ...
        if any(weight > 0 for weight in self._next_item_loss_weight.values()):
            predictions = self.predict_next(
                ...
                logits_fields_mapping={k: f"_{k}_logits" for k in self._categorical_fields},
            )  # (B, L).
            ...
            for field in self.data_fields:
                if field in self._categorical_fields:
                    fixed_predictions[field] = (
                        predictions.payload[f"_{field}_logits"][:, :-1].flatten(0, 1).unsqueeze(1)
                    )  # (BL, 1, C).
                else:
            ...
            for field in self.data_fields:
                if field == self._timestamps_field:
                    ...
                    fixed_inputs[field] = (
                        inputs.payload[field][:, 1:].flatten(0, 1).unsqueeze(1).repeat(1, 2)
                    )

            fixed_inputs = PaddedBatch(
                fixed_inputs,
                torch.full([b * (l - 1)], 2, device=inputs.device),
            )  # (BL, 2).
    ...
    def predict_next(self, outputs, states, fields=None, logits_fields_mapping=None):
        ...
        if adapters - {"head"}:
            ...
            reshaped_outputs = PaddedBatch(
                outputs.payload.reshape(b * l, self._k, self._next_item.input_size),
                torch.full([b * l], self._k, device=outputs.device),
            )  # (BL, K, P).
            states = states.reshape(len(states), b * l, 1, -1) if states is not None else None  # (N, BL, 1, D).
            predictions = self._next_item.predict_next(
                reshaped_outputs, states, fields=fields, logits_fields_mapping=logits_fields_mapping
            )  # (BL, K) or (BL, K, C).

            sequences = PaddedBatch(
                {k: v.reshape(b, l, self._k, *v.shape[2:]) for k, v in predictions.payload.items()},
                lengths,
            )  # (B, L, K) or (B, L, K, C).
            ...
            roll_log_not_presence = torch.cat(
                [torch.zeros_like(log_not_presence[..., :1]), log_not_presence[..., :-1]], -1
            )  # (B, L, K).
            ...
            indices = {}
            presence_mask = sequences.payload[PRESENCE]
            if "first" in adapters:
                indices["first"] = (
                    torch.arange(presence_mask.shape[-1] - 1, -1, -1, device=presence_mask.device) * presence_mask
                ).argmax(-1)  # (B, L).
            if "mode" in adapters:
    ...
    def predict_next_k(self, outputs, states, fields=None, logits_fields_mapping=None):
        ...
        states = states.reshape(len(states), b * l, 1, -1) if states is not None else None  # (N, BL, 1, D).
        next_values = self._next_item.predict_next(
            outputs, states, fields=fields, logits_fields_mapping=logits_fields_mapping
        )  # (BL, K) or (BL, K, C).
        ...
        sequences = PaddedBatch(
            {k: v.reshape(b, l, self._k, *v.shape[2:]) for k, v in next_values.payload.items()},
            lengths,
        )
        self.revert_delta_and_sort_time_inplace(sequences)
        return sequences
    ...
    def select_subset(self, batch, indices=None):
        ...
        if isinstance(payload, torch.Tensor):
            ...
            payload = {k: (self.select_subset(v, indices) if k in batch.seq_names else v) for k, v in payload.items()}
    ...
    def match_targets(self, outputs: PaddedBatch, targets: PaddedBatch):
        ...
        b, l = outputs.shape
        n_targets = targets.payload[targets.seq_names[0]].shape[2] - 1
        assert n_targets > 0
        ...
        targets_batch = PaddedBatch(
            targets, torch.full([b * l], 1 + n_targets, dtype=torch.long, device=device)
        )
        outputs_batch = PaddedBatch(outputs, torch.full([b * l], 1, dtype=torch.long, device=device))

        states = None
        losses, _ = self._next_item(targets_batch, outputs_batch, states, reduction="none")  # (BL, T, K).

        zero_presence_target = PaddedBatch({"_presence": torch.zeros_like(targets["_presence"])}, targets_batch.seq_lens)
        ...
        costs.masked_fill_(out_horizon_mask.unsqueeze(2), max_cost + 1)

        b, l, k, t = costs.shape
        costs_cpu = costs.reshape(b * l, k, t).detach().cpu()
        matches = batch_linear_assignment(costs_cpu).to(device).reshape(b, l, k)
        ...
        match_mask = ~invalid_mask
        matches.masked_fill_(invalid_mask, -1)

        n_matches = (matches >= 0).sum().item()
        n_predictions = lengths.sum().item() * self._k
        n_targets_valid = out_horizon_mask.numel() - out_horizon_mask.sum().item()

        metrics["prediction_match_rate"] = n_matches / max(n_predictions, 1)
        metrics["target_match_rate"] = n_matches / max(n_targets_valid, 1)
    ...
    def get_subset_matching(self, inputs, outputs):
        ...
        l2 = outputs.shape[1]
        n_targets = target_windows.payload[target_windows.seq_names[0]].shape[2] - 1
        if (l2 == 0) or (n_targets == 0):
            matching = PaddedBatch(
                torch.full([b, l2, self._k], -1, dtype=torch.long, device=inputs.device),
                target_windows.seq_lens,
            )
            return indices, matching, {}, {}

        matching, losses, metrics = self.match_targets(outputs, target_windows)
